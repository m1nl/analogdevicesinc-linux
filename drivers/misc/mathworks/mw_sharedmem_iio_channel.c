...
static int devm_mw_sharedmem_configure_buffer(struct iio_dev *indio_dev, enum iio_buffer_direction direction)
{
	struct mw_sharedmem_iio_chandev *mwchan = iio_priv(indio_dev);
	struct iio_buffer *buffer;
	int status;
...
static int devm_mw_sharedmem_iio_register(struct iio_dev *indio_dev, enum iio_buffer_direction direction) {
	...
	status = devm_mw_sharedmem_configure_buffer(indio_dev, direction);
	if (status){
		return status;
	}
